...
@WebServlet(name = "GetFifteen", value = "/GetFifteen")
public class GetFifteen extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String url = "https://devapi.qweather.com/v7/weather/15d?";
        String cityID = request.getParameter("cityID");
        JSONObject json = HttpClientUtil.getJSON(url, cityID);
        if(json.getString(Status.CODE).equals(Status.SUCCESS)) {
            JSONArray array = Weather.dealWithfxDate(json.getJSONArray("daily"));
            out.print(array);
        }else {
